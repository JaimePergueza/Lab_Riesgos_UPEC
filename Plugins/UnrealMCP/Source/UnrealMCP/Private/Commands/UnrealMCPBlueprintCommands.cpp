...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleAddComponentToBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!ComponentClass && !ComponentType.EndsWith(TEXT("Component")))
    {
        ...
        ComponentClass = FindFirstObjectSafe<UClass>(*ComponentTypeWithSuffix);
    }
    ...
    if (!ComponentClass && !ComponentType.StartsWith(TEXT("U")))
    {
        ...
        ComponentClass = FindFirstObjectSafe<UClass>(*ComponentTypeWithPrefix);
        
        // Try with both prefix and suffix
        if (!ComponentClass && !ComponentType.EndsWith(TEXT("Component")))
        {
            FString ComponentTypeWithBoth = TEXT("U") + ComponentType + TEXT("Component");
            ComponentClass = FindFirstObjectSafe<UClass>(*ComponentTypeWithBoth);
        }
    }
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleSetPhysicsProperties(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Collect target components (specific or all)
    TArray<UPrimitiveComponent*> TargetComponents;
    for (USCS_Node* Node : Blueprint->SimpleConstructionScript->GetAllNodes())
    {
        if (!Node) continue;
        bool bMatchName = ComponentName.IsEmpty() || Node->GetVariableName().ToString() == ComponentName;
        if (bMatchName)
        {
            UPrimitiveComponent* Prim = Cast<UPrimitiveComponent>(Node->ComponentTemplate);
            if (Prim) TargetComponents.Add(Prim);
        }
    }
    ...
    if (TargetComponents.Num() == 0)
    {
        FString Msg = ComponentName.IsEmpty()
            ? TEXT("No primitive components found in blueprint")
            : FString::Printf(TEXT("Component not found: %s"), *ComponentName);
        return FUnrealMCPCommonUtils::CreateErrorResponse(Msg);
    }

    for (UPrimitiveComponent* PrimComponent : TargetComponents)
    {
        // Set collision profile
        if (Params->HasField(TEXT("collision_profile")))
        {
            FString Profile = Params->GetStringField(TEXT("collision_profile"));
            PrimComponent->SetCollisionProfileName(FName(*Profile));
        }

        if (Params->HasField(TEXT("collision_enabled")))
        {
            bool bEnabled = Params->GetBoolField(TEXT("collision_enabled"));
            PrimComponent->SetCollisionEnabled(bEnabled ? ECollisionEnabled::QueryAndPhysics : ECollisionEnabled::NoCollision);
        }
    ...
    ResultObj->SetNumberField(TEXT("components_modified"), TargetComponents.Num());
    return ResultObj;
}
